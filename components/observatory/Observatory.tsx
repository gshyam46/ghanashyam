    ...
    <div className="observatory" data-palette={palette} data-motion={paused ? "still" : "flow"}>
      ...
      <main id="main-content">
        <section className="chapter signal-section" id="signal" aria-labelledby="signal-title">
          ...
          <div className="hero-bottom"><span>AI INTEGRATION <i /> AGENTIC SYSTEMS <i /> OBSERVABILITY</span><span className="hero-note">A little curiosity goes a long way. <span>↘︎</span></span></div>
          ...
          <div id="project-exhibit" className="project-exhibit" role="tabpanel" aria-labelledby={`tab-${currentProject.id}`}>
            <div className="project-story" key={currentProject.id}>
              ...
              <div className="project-actions">
                ...
                {currentProject.live && <a className="text-link" href={currentProject.live} target="_blank" rel="noreferrer" onClick={() => audio.chime(projectIndex)}>{currentProject.liveLabel} <ArrowUpRight size={18} /></a>}
                {currentProject.source && <a className="text-link" href={currentProject.source} target="_blank" rel="noreferrer" onClick={() => audio.chime(projectIndex)}>{currentProject.sourceLabel} <ArrowUpRight size={18} /></a>}
              </div>
            </div>
            <div className="project-preview"><ProjectDemo project={currentProject.id} onInteract={() => audio.chime(projectIndex)} /></div>
          </div>
          <div className="section-footnote"><span>SELECT A SYSTEM. FOLLOW THE SIGNAL.</span><span>INDEPENDENT PROJECTS & ONGOING EXPLORATIONS ↗︎</span></div>
        ...
        <section className="chapter practice-section" id="practice" aria-labelledby="practice-title">
          ...
          <div className="practice-layout">
            <div className="practice-intro"><h2 id="practice-title">See the whole.<br /><em>Care for the details.</em></h2><p>I work where software engineering meets applied AI. My interest is in the connective tissue: how systems communicate, how agents act, and how we understand what happens in between.</p><dl className="practice-toolkit" aria-label="Selected skills">{skillGroups.map(group => <div key={group.name}><dt>{group.name}</dt><dd>{group.tools}</dd></div>)}</dl><a className="text-link" href={profile.github} target="_blank" rel="me noreferrer">Explore my GitHub <ArrowUpRight size={17} /></a></div>
        ...
        <section className="chapter contact-section" id="contact" aria-labelledby="contact-title">
          ...
          <div className="contact-content"><p className="contact-kicker">HAVE SOMETHING INTERESTING IN MIND?</p><h2 id="contact-title">Let’s build<br /><em>together.</em><ArrowUpRight className="contact-arrow" /></h2><p className="contact-description">A system to untangle. An idea to bring to life.<br />A conversation about what comes next.</p>
            <div className="contact-buttons"><a href={profile.email ? `mailto:${profile.email}` : profile.linkedin} target={profile.email ? undefined : "_blank"} rel="noreferrer" className="primary-button">Start a conversation {profile.email ? <Mail size={18} /> : <ArrowUpRight size={18} />}</a><a className="social-button" href={profile.github} target="_blank" rel="me noreferrer" aria-label="Ghanashyam on GitHub"><Github size={20} /></a><a className="social-button" href={profile.linkedin} target="_blank" rel="me noreferrer" aria-label="Ghanashyam on LinkedIn"><Linkedin size={20} /></a></div>
          </div>
